...
elseif ($mod == 'periode_tambah') {
    ...
    elseif ($db->get_row("SELECT * FROM tb_periode WHERE nama_periode='$nama_periode' AND kode_periode<>'$kode_periode'"))
        ...
        foreach ($_POST['nilai'] as $key => $val) {
            $db->query("INSERT INTO tb_prediksi(kode_periode, kode_jenis, nilai) VALUES ('$kode_periode', '$key', '$val')");
        }
    ...
    if ($nama_periode == '')
        print_msg("Field yang bertanda * tidak boleh kosong!");
    elseif ($db->get_row("SELECT * FROM tb_periode WHERE nama_periode='$nama_periode' AND kode_periode<>'$_GET[ID]'"))
        print_msg("Periode sudah ada!");
    else {
        $db->query("UPDATE tb_periode SET nama_periode='$nama_periode' WHERE kode_periode='$_GET[ID]'");
        foreach ($_POST['nilai'] as $key => $val) {
            $db->query("UPDATE tb_prediksi SET nilai='$val' WHERE id='$key'");
        }
        redirect_js("index.php?m=periode");
    }
...
elseif ($mod == 'jenis_tambah') {
    ...
    elseif ($db->get_results("SELECT * FROM tb_jenis WHERE kode_jenis='$kode_jenis'"))
        ...
        $db->query("INSERT INTO tb_prediksi(kode_periode, kode_jenis) SELECT kode_periode, '$kode_jenis'  FROM tb_periode");
